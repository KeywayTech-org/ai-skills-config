...
- **[references/three-adapter.md](references/three-adapter.md)** — Three.js and WebGL adapter patterns for HyperFrames. Read when creating deterministic Three.js scenes, WebGL canvas layers, AnimationMixer timelines, camera motion, shader-driven visuals, or any canvas render that responds to `hf-seek` events.
- **[references/lottie-adapter.md](references/lottie-adapter.md)** — Lottie and dotLottie adapter patterns for HyperFrames. Read when embedding lottie-web JSON animations, `.lottie` files, or `@lottiefiles/dotlottie-web` players. Covers `window.__hfLottie` registration, lottie-web pattern, dotLottie pattern, multi-animation sync.
- **[references/animejs-adapter.md](references/animejs-adapter.md)** — Anime.js adapter patterns for HyperFrames. Read when the user specifically asks for Anime.js animations or timelines. Covers `window.__hfAnime` registration, basic and timeline patterns. For complex scene sequencing, prefer GSAP.
- **[references/css-animations-adapter.md](references/css-animations-adapter.md)** — CSS keyframe animation adapter for HyperFrames. Read when authoring CSS `@keyframes`, `animation-delay` timing, `animation-fill-mode`, or CSS-only decorative motion (shimmer, glow, pulse, background loops). For scene choreography, prefer GSAP.
- **[references/waapi-adapter.md](references/waapi-adapter.md)** — Web Animations API adapter for HyperFrames. Read when using `element.animate()` for native browser keyframes with no GSAP dependency. Covers `document.getAnimations()` seeking, `KeyframeEffect` timing, fill modes.
- **[references/typegpu-adapter.md](references/typegpu-adapter.md)** — TypeGPU and raw WebGPU adapter for HyperFrames. Read when creating GPU-rendered compositions: WGSL fragment shaders, compute pipelines, liquid glass effects, particle systems. Note: HTML/text elements in the same composition should still use GSAP registered synchronously before any `await`.
